...
func (em *Manager) Init(p *killer.Killer) {
	...
	em.SharedModel = model.GunRobotModel
	em.SharedAnimations = model.GunRobotAnimation
...
func (em *Manager) Mutate(
	...
	for i := 0; i < len(em.Enemies); i++ {
		...
		if em.Enemies[i].JustDied {
			em.DeathPositions = append(em.DeathPositions, em.Enemies[i].Position)
			em.Enemies[i].JustDied = false
		}
		if em.Enemies[i].KilledByPlayer {
			em.PlayerBulletKillCount++
			em.Enemies[i].KilledByPlayer = false
		}
	}
...
const indicatorMaxRange = 60.0

func (em *Manager) DrawOffscreenIndicators(p *killer.Killer) {
	const margin = float32(36)
	const arrowSize = float32(10)

	w := float32(util.VirtualWidth)
	h := float32(util.VirtualHeight)
	cx := w / 2
	cy := h / 2

	maxRangeSq := float32(indicatorMaxRange * indicatorMaxRange)

	for _, e := range em.Enemies {
		if !e.IsAlive() || e.IsHiddenFromKiller {
			continue
		}
		if rl.Vector3DistanceSqr(e.Position, p.Position) > maxRangeSq {
			continue
		}
		...
		tip := rl.Vector2{X: edgeX + cos*arrowSize, Y: edgeY + sin*arrowSize}
		baseAngle1 := angle + math.Pi*0.75
		baseAngle2 := angle - math.Pi*0.75
		left := rl.Vector2{
			X: edgeX + float32(math.Cos(float64(baseAngle1)))*arrowSize,
			Y: edgeY + float32(math.Sin(float64(baseAngle1)))*arrowSize,
		}
		right := rl.Vector2{
			X: edgeX + float32(math.Cos(float64(baseAngle2)))*arrowSize,
			Y: edgeY + float32(math.Sin(float64(baseAngle2)))*arrowSize,
		}

		rl.DrawTriangle(tip, right, left, rl.NewColor(220, 30, 30, 220))
		rl.DrawTriangleLines(tip, right, left, rl.Black)
	}
}

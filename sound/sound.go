...
func Init() {
	...
	ShotNew = util.LoadSoundFromEmbedded("shot_new_trimmed.mp3")
	rl.SetSoundVolume(ShotNew, 0.5)
	FootStep = util.LoadSoundFromEmbedded("data_pion-st1-footstep-sfx-323053.mp3")
	Win = util.LoadSoundFromEmbedded("win.mp3")
}

func SetGlobalPitch(pitch float32) {
	rl.SetSoundPitch(ShotgunSound, pitch)
	rl.SetSoundPitch(ReloadingSound, pitch)
	rl.SetSoundPitch(Track, pitch)
	rl.SetSoundPitch(ShotNew, pitch)
	rl.SetSoundPitch(FootStep, pitch)
}
